<script setup lang="ts">
  ...
  const columns: ColumnDef<{ id: number; name: string; createdAt: Date }, unknown>[] = [
    { accessorKey: 'id', header: 'Id', enableSorting: true, cell: ({ row }) => h('p', {}, row.original.id) },
    { accessorKey: 'name', header: 'Name', enableSorting: true, cell: ({ row }) => h('p', {}, row.original.name) },
...
<template>
  <div class="space-y-4">
    <div class="flex items-center gap-3">
      <Button test-id="" outline @click="toggleDark()">Toggle Dark Mode</Button>
      <RouterLink
    ...
    <div>
      <Avatar>
        ...
        <AvatarFallback class="bg-primary-500 rounded-full"> HK </AvatarFallback>
      </Avatar>
    </div>
    <div>

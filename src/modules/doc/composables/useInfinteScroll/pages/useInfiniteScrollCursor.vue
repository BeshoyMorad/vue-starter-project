<script setup lang="ts">
  ...
  const {
    data: cursorData,
    isFetchingNextPage: cursorIsFetchingNextPage,
    hasMore: cursorHasMore,
    fetchNextPage: cursorFetchNextPage,
  } = useDataInfiniteScroll<MockUser, UserFilters>({
    queryKey: ['docs-infinite-cursor-users'],
    endpoint: '/users',
    paginationType: 'cursor',
    queryOptions: {
      queryFn: ({ pageParam }: { pageParam: number }) => {
        return getMockCursorUsers({
          page: pageParam,
          limit: 4,
        });
      },
...
<template>
  <Card
    ...
    header="Usage"
    :code-arr="[
      { code: USE_DATA_INFINITE_SCROLL_CURSOR_CODE, header: 'script' },
      { code: USE_DATA_INFINITE_SCROLL_CURSOR_COMPONENT_CODE, header: 'template' },
    ]"
  >
    <template #preview>
      <InfiniteScroll
        ...
        :has-more="cursorHasMore"
        :loading="cursorIsFetchingNextPage"
        @load-more="cursorFetchNextPage"
      >
